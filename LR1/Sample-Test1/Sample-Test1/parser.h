...
public:
    ...
    {
    public:
        int_least16_t comm_id, width, height, x0, y0, x1, y1, w, h, radius, radius_x, radius_y, R, G, B, font, length, orientation;
        string text;
        bool check;
        ...
        else
        {
            ...
            else
            {
                ...
                case DRAW_TEXT:
                    ...
                    break;
                case DRAW_IMAGE:
                    if (my_count != 5) { my_answer.error += "Не вірно набрана команда"; my_answer.check = false; break; }
                    else
                    {
                        my_answer.x0 = strtol(end, &end, 10);
                        my_answer.y0 = strtol(end, &end, 10);

            ...
            if (bsize > 0)
            {
                ...
                for (int k = 0; k <= i; k++)
                {
                    ...
                    if (my_parser.my_answer.check)
                    {
                        ...
                        case DRAW_TEXT:
                        {
                            ...
                            double bg_w = (my_parser.my_answer.font * my_parser.my_answer.text.length()) / 1.66;
                            x_ = text_to_line(&window, my_parser.my_answer.text, my_parser.my_answer.x0, my_parser.my_answer.y0, my_parser.my_answer.R, my_parser.my_answer.G, my_parser.my_answer.B, my_parser.my_answer.font);
                            break;
                        }
                        case DRAW_IMAGE:
                        {
                ...
                if (my_parser.my_answer.check == false)
                {
                    ...
                    if (my_parser.my_answer.comm_id == DRAW_CHAR || my_parser.my_answer.comm_id == DRAW_TEXT)
                    {
                        ...
                        if (my_parser.my_answer.comm_id == GET_WIDTH)
                        {
                            string b_w = to_string(my_parser.my_answer.width);
                            strcpy(buff_, b_w.c_str());
                            cout << buff_ << endl;
                            int bsize_;
                            bsize_ = strlen(buff_);
                            sendto(Socket, &buff_[0], bsize_, 0, (sockaddr*)&client_addr, sizeof(client_addr));
                        }
                        else if (my_parser.my_answer.comm_id == GET_HEIGHT)
                        {
                            string b_h = to_string(my_parser.my_answer.height);
                            strcpy(buff_, b_h.c_str());
                            cout << buff_ << endl;
                            int bsize_;
                            bsize_ = strlen(buff_);
                            sendto(Socket, &buff_[0], bsize_, 0, (sockaddr*)&client_addr, sizeof(client_addr));
